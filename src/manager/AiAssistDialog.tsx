...
## 오래 걸리거나 백그라운드로 도는 작업 (중요)
몇 초 이상 걸리거나 스크립트가 끝난 뒤에도 백그라운드에서 계속 도는 작업(타이머, 반복 호출, 여러 개의 osascript/curl 등)이면 "지금 켜진 건지" 사용자가 바로 알 수 있어야 합니다.
- 작업을 백그라운드로 넘길 때(\`&\`, \`disown\`, \`nohup\` 등)는 넘긴 직후 바로 "OO을 시작했어요" 같은 문장을 echo해서 그게 스크립트의 마지막 줄(=알림 문구)이 되게 하세요. 결과가 나와야만 시작 여부를 알 수 있는 구조는 피하세요.
- curl 등 네트워크 호출에는 반드시 타임아웃(예: \`--max-time 5\`)을 넣어서, 응답이 없을 때 스크립트가 멈춘 건지 원래 그런 건지 알 수 없는 상태가 되지 않게 하세요.

## 실패 원인을 진단할 수 있게 (중요)
- 외부 명령(curl, osascript 등)의 종료 코드/상태 코드를 확인하지 않고 넘어가면, 그 명령이 실패해도 스크립트는 exit 0으로 끝나 성공한 것처럼 보일 수 있습니다. 그런 조용한 실패가 없게 하세요.
- 이 앱은 모든 실행의 stdout/stderr를 시각별로 자동 로그 파일에 남깁니다. 스크립트에서 직접 로그 파일을 만들거나 관리할 필요는 없고, 진행 상황이나 중간 값을 echo로 남기기만 하면 그게 그대로 로그에 기록됩니다.
