...
- **gh CLI 계정이 두 개 등록되어 있다**: `SteveKim0513`(public 저장소용, SSH 별칭 `github.com-stevekim`)와 `imaginefutures`(SSH 별칭 `github.com-imaginefutures`). 이 프로젝트는 `SteveKim0513/mac-shortcut-manager`(public)를 쓴다. **활성 계정이 세션 사이에 저절로 `imaginefutures`로 돌아가 있는 경우가 반복 관찰됨** — 한 번은 다른 계정 토큰으로 `npm run release`를 돌리다가 GitHub API가 404(권한 없음을 이렇게 감춤)를 뱉은 적 있다. `npm run release`나 `gh` 관련 작업 전에는 매번 `gh api user --jq '.login'`으로 확인하고, `SteveKim0513`이 아니면 `gh auth switch --hostname github.com --user SteveKim0513`부터 실행한다.
